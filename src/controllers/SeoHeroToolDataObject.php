<?php
    ...
    class SeoHeroToolDataObject extends DataExtension
    {
            ...
            for ($i=0; $i< count($entry); $i++) {
                ...
                if ($elementIsVariable) {
                    if (strpos($actualElement, '()')) {
                        ...
                        if($content === ''){
                            $ClassName = $this->owner->URLSegment;
                            if (method_exists($ClassName, $actualElement)) {
                                $content = $ClassName::{$actualElement}();
                            }
                        }
                    } elseif (strpos($actualElement, '.')) {
                        $HasOneArray = explode(".", $actualElement);
        ...
        {
            return $this->getBetterMetaDescription();
        }
         ...
         */
        public function getBetterMetaDescription()
        {
            if ($this->owner->MetaDescription != '') {
                ...
                $classname = $this->owner->ClassName;
                $urlname = $this->owner->URLSegment;
                $yamlsettings = config::inst()->get('SeoHeroToolDataObject', $classname);
                if(!$yamlsettings){
                    $yamlsettings = config::inst()->get('SeoHeroToolDataObject', $urlname);
                }
